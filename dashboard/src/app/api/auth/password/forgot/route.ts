// POST /api/auth/password/forgot — request a password-reset email (Team tier).
...
// account. The reset token travels only inside the email the gateway sends
...
export async function POST(req: Request) {
  ...
  try {
    const gwRes = await fetch(`${GATEWAY_HTTP_URL}/api/auth/password/forgot`, {
      method: 'POST',
      headers: {
