/**
 * savings-card.ts — the viral "tokens saved" share card (dashboard mirror).
 *
 * ⚠ BYTE-FOR-BYTE MIRROR of runtime/src/monitoring/savings-card.ts — the two
 * packages don't share code (same convention as db.ts's read mirrors). The
 * canonical, unit-tested copy lives in the runtime; keep them in sync. Renders a
 * dependency-free 1200×630 (OG-standard) SVG: "myAI saved me N tokens / $X this
 * month". Pure string builder — safe to render in the /savings/card route.
 */
