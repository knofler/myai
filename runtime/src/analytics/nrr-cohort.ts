...
// tracked per signup-month cohort over time. Distinct from analytics/revenue.ts,
// which reports a single trailing-window NRR number; this module answers the
...
//
// Pure, no I/O (same discipline as revenue.ts) — fed a starting cohort (each
// account's MRR in its first paying month) plus, for each later month, a
// lookup of each account's MRR then (absent/0 ⇒ churned by that month). The
// caller assembles those snapshots from Stripe + tenant events; the math here
// is exact and unit-testable. Mirrored in dashboard/src/lib/nrr-cohort.ts.

/** One account's MRR contribution in the cohort's first paying month. */
export interface CohortAccountStart {
  tenantId: string;
  startingMrr: number;
}

/** A later month's MRR-by-tenant lookup for a given cohort. */
